...
Fork [this GitHub repository](https://github.com/isaru66/TechWorkshop-L300-AI-Apps-and-Agents) to your account. You will need to fork the repo in order to modify and use GitHub Actions workflows in later tasks.
...
```bash
az login
az group create --name tw-ai-agents-g01 --location swedencentral
```

Alternatively, you may choose to do this from [the Azure portal](https://portal.azure.com/). In the search bar at the top, search for and select **Resource groups**. Then, select the **+ Create** button to create a new resource group. Choose your subscription and give the resource group a name such as `tw-ai-agents-g01`.
...
<summary><strong>Expand this section to view the solution</strong></summary>
 ...
 In Visual Studio Code, open a new Terminal and ensure that you are in the root of your workspace. First, ensure that you are logged into the Azure CLI. You can do so using `az login` on a local machine or `az login --use-device-code` in a Codespace VM.
...
az deployment group create \
  --resource-group tw-ai-agents-g01 \
...
Then choose ```none``` as the parameter file.
