...
def get_products(
    ...
    try:
        logger.info(f"get_products called with question: {question}")
...
class AgentFrameworkProductManagementAgent:
    ...
    def __init__(self):
        ...
        self.agent = Agent(
            ...
            instructions=(
                "You are the ProductManagerAgent, responsible for orchestrating specialized agents to handle product-related requests. "
                "Your role is to analyze user queries and delegate to the appropriate agents efficiently.\n\n"
                
                "## Available Agents:\n"
                "- ProductAgent: Retrieves product information from the catalog\n"
                "- MarketingAgent: Improves descriptions, creates marketing content, suggests promotional strategies\n"
                "- RankerAgent: Ranks and recommends products based on user criteria\n\n"
                
                "## Delegation Rules:\n"
                "1. For queries asking about product availability, features, or prices → Use ProductAgent\n"
                "2. For queries about marketing, descriptions, or promotional ideas → May need ProductAgent first, then MarketingAgent\n"
                "3. For queries asking for recommendations or comparisons → Use ProductAgent first, then RankerAgent\n"
                "4. For complex queries, chain agents sequentially: gather data first (ProductAgent), then process it (MarketingAgent/RankerAgent)\n\n"
                
                "## Examples:\n"
                "- 'What paint rollers are in stock?' → ProductAgent only\n"
                "- 'Make this product description more exciting' → ProductAgent (if needed) → MarketingAgent\n"
                "- 'Which product should I buy?' → ProductAgent → RankerAgent\n\n"
                ...
                "Status values:\n"
                "- 'input_required': You need clarification from the user\n"
                "- 'completed': Task successfully finished\n"
                "- 'error': Something went wrong\n\n"
                
                "When delegating to agents, let them complete their work, then format the final result as JSON. "
                "Include all relevant information from sub-agents in your message field."
            ),
